...
public class AgentInscritController {
    ...
    private final AgentInscritRepo agentInscritRepository;
...
// ✅ Enregistrement d’un agent en attente sans traitement du port
    ...
    public ResponseEntity<?> register(@RequestBody AgentInscriptionRequest request) {
        // Validation de l'ICE si besoin
        if (request.getIce() == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("ICE manquant");
        }
        ...
        agent.setICE(request.getIce()); // ✅ utilise la valeur reçue
        agent.setPassword(encodedPassword);
        agent.setPortDemande(request.getPortId() != null ? portRepository.findById(request.getPortId()).get() : null);
        agent.setStatut(AgentInscrit.Statut.EN_ATTENTE);
        agent.setDateInscription(new Date());

        // Enregistrer l'agent dans la base
    ...
    public ResponseEntity<?> accepterAgentInscrit(@PathVariable Integer id) {
        try {
            Optional<AgentInscrit> optional = agentInscritRepository.findById(Long.valueOf(id));
            if (optional.isEmpty()) {
    ...
    public ResponseEntity<?> supprimerAgentInscrit(@PathVariable Integer id) {
        if (agentInscritRepository.existsById(Long.valueOf(id))) {
            agentInscritRepository.deleteById(Long.valueOf(id));
